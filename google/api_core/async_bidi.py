...
class AsyncBidiRpc(object):
    ...
    def open(self):
        ...
        call = self._loop.run_until_complete(self._start_rpc(iter(request_generator), metadata=self._rpc_metadata))
...
class AsyncResumableBidiRpc(AsyncBidiRpc):
    ...
    def __init__(
        ...
        super(AsyncResumableBidiRpc, self).__init__(start_rpc, initial_request, metadata)
    ...
    def close(self):
        self._finalize(None)
        super(AsyncResumableBidiRpc, self).close()
...
class AsyncBackgroundConsumer(object):
